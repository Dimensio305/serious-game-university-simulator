...
public class Projet
{
    ...
    // Données communes aux deux méthodes
    private static readonly string[] NomsProjets = { 
        "Campus Connect", 
        "Espace Collaboratif", 
        "Laboratoire Innovant", 
        "Bibliothèque Numérique", 
    ...
    private static readonly string[] DescriptionsProjets = { 
        "Développement d'une plateforme de communication pour le campus", 
        "Création d'espaces collaboratifs pour étudiants et chercheurs", 
        "Installation d'équipements de pointe pour la recherche", 
        "Numérisation des ressources de la bibliothèque", 
    ...
    // Méthode commune pour générer un projet aléatoire
    private static Projet GenererunProjetAleatoire()
    {
        Random random = new Random();
        int p = random.Next(NomsProjets.Length);
        return new Projet(NomsProjets[p], DescriptionsProjets[p]);
    }

    // Méthode pour générer plusieurs projets aléatoires
    public static List<Projet> GenererProjetsAleatoires()
    {
        List<Projet> projets = new List<Projet>();
        Random random = new Random();
        ...
        for (int i = 0; i < nombreProjets; i++)
        {
            projets.Add(GenererProjetAleatoire());
        }
